...
- **看板組成**：建議各國每日各推 **3 項**（共 9 項），跨難度平衡。玩家可全做、也可只挑想做的——這正是「均衡 vs 專精」的玩家選擇。每日推出數量待確認。
- **重置時間**：每日當地時間 **凌晨 3:00**（避開熬夜者跨日，多數人此時已就寢）。本地以裝置時間判斷；有 server 後改以 server time 為準。
- 任務難度分三階：`簡單 / 普通 / 挑戰`，影響基礎分（見 §6）。
- 任務內容以 **資料表（ScriptableObject / JSON / server `taskTemplates`）** 維護，每筆標記所屬王國，方便新增與節慶限定任務。
...
```
單一任務貢獻分 = 基礎分(依難度) × 連勝加成

基礎分：簡單 10 / 普通 15 / 挑戰 25
連勝加成：1 + min(連續達標天數 × 0.05, 0.50)   // 每天 +5%，上限 +50%
```
...
- **連勝（Streak）**：以「當日是否達成每日目標（如完成任意 N 項任務，不限王國）」判定延續，中斷則歸零。門檻待確認。
- **每日上限**：建議設個人貢獻分日上限，避免單日爆量破壞王國平衡。
- **王國總分**：所有玩家在「該國任務」上累積的貢獻分總和。

平衡性注意：各國每日任務的**數量與難度配比要相當**，否則某國天生較容易被完成、累積分數偏高。
...
4. **連勝門檻**：需拍板（全完成 or 5 取 3）。重置時間已定為 **凌晨 3:00**。
